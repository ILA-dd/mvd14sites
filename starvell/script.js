...
const STORAGE_LAST_SPIN = 'starpets_mm2_last_spin';
const STORAGE_HISTORY = 'starpets_mm2_win_history';
const STORAGE_USERNAME = 'starpets_mm2_username';
const STORAGE_VISITOR_ID = 'starpets_mm2_visitor_id';
...
function showGroupModal() {
  ...
  document.getElementById('groupManualBtn').addEventListener('click', function() {
    groupJoined = true;
    try { localStorage.setItem('starpets_group_joined', '1'); } catch(e) {}
...
(function restoreGroupState() {
  try { if (localStorage.getItem('starpets_group_joined') === '1') groupJoined = true; } catch(e) {}
})();
...
try {
  ...
  console.error('StarPets init error:', err);
}
